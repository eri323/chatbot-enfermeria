...
import { getLaboratorios, verificarDisponibilidad, crearReservacion, cancelarReservacion, verificarFestivo } from "../services/api";

export default function Chatbot({ usuario }) {
    const [mensajes, setMensajes] = useState(() => {
        const guardados = localStorage.getItem("chat_mensajes");
        return guardados ? JSON.parse(guardados) : [
            { texto: "¡Hola! Soy el asistente de reservas de laboratorios. ¿En qué te puedo ayudar?", tipo: "bot" }
        ];
    });
    ...
    const iniciarConsulta = async () => {
        ...
        if (esFindeSemana(fechaSeleccionada)) {
            agregarMensaje(`❌ El ${fechaSeleccionada} es fin de semana, no hay disponibilidad.`, "bot");
            return;
        }
        try {
            const res = await verificarFestivo(fechaSeleccionada);
            if (res.data.esFestivo) {
                agregarMensaje(`❌ El ${fechaSeleccionada} es festivo, no hay disponibilidad.`, "bot");
                return;
            }
        } catch (err) {}

        const lista = laboratorios.map(l => `• ${l.nombre}`).join("\n");
        agregarMensaje(`📅 Fecha seleccionada: ${fechaSeleccionada}\n¿Qué laboratorio quieres consultar?\n${lista}`, "bot");
    ...
    const iniciarReserva = async () => {
        ...
        if (esFindeSemana(fechaSeleccionada)) {
            agregarMensaje(`❌ El ${fechaSeleccionada} es fin de semana, no puedes reservar.`, "bot");
            return;
        }
        try {
            const res = await verificarFestivo(fechaSeleccionada);
            if (res.data.esFestivo) {
                agregarMensaje(`❌ El ${fechaSeleccionada} es festivo, no puedes reservar.`, "bot");
                return;
            }
        } catch (err) {}
    ...
    const manejarPaso = async (mensaje) => {
        ...
        if (paso === "consulta_lab") {
            ...
            const texto = horariosDisponibles.map(h =>
                `${h.disponible ? "✅" : "❌"} ${h.inicio} - ${h.fin}`
            ).join("\n");
            ...
            const texto = horariosDisponibles.map(h => `• ${h.inicio} - ${h.fin}`).join("\n");
            agregarMensaje(`Horarios disponibles para ${lab.nombre}:\n${texto}\n\n¿Qué horario prefieres? (ej: 08:00-10:00)`, "bot");
            ...
            setEstado({ paso: "reserva_estudiantes", datos: { ...datos, materials: mensaje } });
            ...
            if (isNaN(num_students) || num_students <= 0) {
                agregarMensaje("Por favor ingresa un número válido de estudiantes.", "bot");
                return;
            }
            try {
            ...
            try {
                const res = await cancelarReservacion(mensaje, { cancelada_por: usuario.id });
                if (res.data.success) {
                    agregarMensaje(`✅ Reserva #${mensaje} cancelada exitosamente.`, "bot");
                } else {
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <div className="px-4 py-2 flex gap-2 flex-wrap items-center border-t">
                ...
                />
                <button onClick={iniciarConsulta} className="bg-green-500 hover:bg-green-600 text-white text-sm px-3 py-1 rounded-lg">
                    Consultar disponibilidad
                </button>
                <button onClick={iniciarReserva} className="bg-blue-500 hover:bg-blue-600 text-white text-sm px-3 py-1 rounded-lg">
                    Reservar laboratorio
                </button>
                <button onClick={iniciarCancelacion} className="bg-red-500 hover:bg-red-600 text-white text-sm px-3 py-1 rounded-lg">
                    Cancelar reserva
                </button>
            </div>
            ...
            <div className="px-4 py-3 border-t flex gap-2">
                ...
                />
                <button onClick={handleEnviar} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-semibold">
                    Enviar
                </button>
            </div>
        </div>
